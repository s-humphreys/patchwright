...
| `GET /api/v1/history` | How the queue moved by month: opened, fixed **with evidence** (`resolved`), left without a fix (`lapsed`), ticketed against unticketed, the estate's risk direction, and the open work items by signal at the end of each period (`risk[].open_by_signal`, each item once under its most severe signal). Needs a [history store](history.md); says `enabled: false` otherwise. With the tracker read, also cycle time (finding to ticket, ticket to In Progress, In Progress to resolved) and tickets raised and closed by the tracker's own dates, marked `source: tracker` and reported as tickets, not resolutions. `?since=90d&bucket=month` |
| `GET /api/v1/history/item?key=<key>` | One work item's record, every event oldest first |
| `GET /api/v1/history/tickets` | Tickets created per UTC day by the tracker's own created date, every day of the range present (zero days included), each day listing its tickets with key, project, title, current status, matched work item and a link to the tracker. Always per day; `?since=90d` and `until` as for `/api/v1/history`. Needs a [history store](history.md); says `enabled: false` otherwise |
