...
function period(label, over = {}) {
  return {
    ...
    opened: 0, resolved: 0, lapsed: 0, reassigned: 0,
...
function body(over = {}) {
  return {
    ...
    history: {
      ...
      movement: [
        period('2026-07'),
        period('2026-08', { opened: 300, resolved: 2, lapsed: 1, resolved_ticketed: 1, resolved_unticketed: 1,
...
test('resolved and lapsed are shown apart and never summed', () => {
  const html = render(body());
  // Across the range: 320 opened, 7 resolved, 9 lapsed. No "16" anywhere as a total.
  assert.match(html, /320 opened · <strong class="ok">7<\/strong> resolved with evidence · <span class="muted">9 lapsed<\/span>/);
  assert.match(html, /Lapsed is never remediation/);
});
...
test('the signal split classifies by the opening state and shows EPSS decay apart', () => {
  ...
  assert.match(html, /EPSS decayed below 0\.5 while open<\/dt><dd>10/);
...
test('periods with nothing in them are dropped except the latest', () => {
  ...
  const movementSection = html.slice(html.indexOf('<h3>Movement</h3>'), html.indexOf('<h3>Total remediation'));
